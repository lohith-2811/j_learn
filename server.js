...
const app = express();
const PORT = process.env.FULL_PORT || 5000; // Different port to avoid conflict with index.js
...
app.post('/signup', async (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(400).json({
      error: 'Password must be at least 8 characters and can include letters, numbers, and special characters (@#$%^&*()+!~)',
      field: 'password'
    });
  }
